...
- **Steps:**
  ...
  3. Add a **watchdog**: if no tokens for N seconds (default 120), classify as `wedge_suspected`, write honest ledger entry, exit non-zero. Do NOT auto-restart the service (state-changing) — report it.
...
- **Design (build exactly this):**
  ...
  - Event types: `run_started`, `node_started`, `node_progress`, `node_finished`, `node_failed`, `node_retried`, `artifact_produced` (node, path/uri, content-hash), `artifact_consumed`, `run_finished`, `run_failed`. Every event: `run_id`, `node_id`, `attempt`, `ts`, plus telemetry fields (`duration_ms`, `tokens_in/out`, `model`, `endpoint`) where applicable.
...
- [ ] A0 verify real offload path  - [ ] A1 windowing helper  - [ ] A2 dev pack  - [ ] A3 knowledge pack  - [ ] A4 agent-support  - [ ] A5 offline assurance  - [ ] A6 sovereignty gradient  - [ ] A7 card schema v2 (after v2 run completes)
