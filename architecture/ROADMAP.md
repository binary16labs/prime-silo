...
| **C** | ✅ shipped | All eight canvases ported into `app/L0/_all/mod/_prime_silo/widgets/<scope>/<id>/`: `text.markdown`, `run.reasoning_trace`, `run.lineage_timeline`, `run.drilldown_table`, `run.frame_inspector`, `kg3d.synoptic_web`, `codegraph.canvas`, `dag.canvas`. Pluggable-renderer hook on graph widgets keeps the door open for Three.js. |
...
| **F2** | open | `pinView` follow-up — persistence half of Phase F. Compose `signView + write to canonical workspace location + lineage emit` into a single browser helper. New runtime endpoint `POST /api/views/pin` (human-only by middleware) writes to `$BENNY_HOME/workspaces/<ws>/views/`. Validates the F chokepoint on a real persistence surface. |
