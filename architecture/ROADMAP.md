...
| **M1** | ✅ shipped | Memory graph in-shell. Memo-Ray declared as a signed `aamp.integration/1` manifest (`manifests/integrations/memoray.integration.json`: data model + per-endpoint payload contracts + process map + config surface). One configurable proxy (`server/lib/memoray_proxy.js`, `/api/memoray`, settings via `MEMORAY_ENABLED`/`MEMORAY_BASE_URL` then the wizard manifest), four surfaces: page (`#/_prime_silo/memory` — `memoray.overview_cards` + `memoray.lineage_graph` widgets, conformance strip, first-class offline/disabled screens), `memory-recall` agent skill, `node space memory` CLI, and the `/api/integration_audit` self-audit (`server/lib/integration_audit.js`, also `node scripts/audit-integrations.mjs`) that probes live reality against the manifest and emits owner-tagged drift findings. |
| **B (Bridge)** | ✅ shipped | One cockpit for the whole mesh. `#/_prime_silo/bridge` ropes the scattered pages into a single calm surface: six modes (Pulse · Memory · Documents · Code 3D · Flows · Runs) down a rail, one stage, and **Benny** (the onscreen agent) in the dock — grounded in the current stage via `bridge-context.js` (`window.__bennyBridgeContext`) + the catalog-loaded `benny-pilot` skill. Golden paths replace copy-paste: Flows does requirement → **Plan** (DAG) → **Run** (observability); Documents does workspace → files → **Ingest** → semantic triples (kg3d) + **Correlate**. Graph modes default to offline-safe 2D SVG with a 2D/3D toggle (three_renderer WebGL). Declared as a signed `aamp.integration/1` manifest (`manifests/integrations/bridge.integration.json`, checks: signature/config_surface/owners), ubiquitous (page + `benny-pilot` skill + `node space bridge <status\|plan\|run\|ingest\|open>`), configurable via `BRIDGE_DEFAULT_MODE`. Incorporates Memo-Ray's `/api/lifelog` activity feed in Pulse (memoray manifest v1.1.0). The old pages stay live as deep-links. |
| **M2** | open | LLM self-maintenance loop. A Benny swarm manifest that consumes the M1 audit JSON, calls `call_model()` (Lemonade local) per drift finding, and drafts fixes into `agent_sandbox/drafts/` for human pinning. The audit contract built in M1 is its input. |
