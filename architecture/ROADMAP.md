...
| **E-runs** | ✅ shipped | Runs explorer — second deterministic-zone shell page at `_prime_silo/runs_explorer/` (routed `#/_prime_silo/runs_explorer`). Reads `/manifests/runs` + `/manifests/runs/<id>`, prefers `manifest_snapshot` for DAG fidelity, colours each task via `dag.canvas` run overlay. Active runs float to the top of the dropdown; `?run_id=` deep-linking supported. 46 tests: `summariseRun`, `buildRunOverlay`, `extractManifestSnapshot`, `sortRunsForDisplay`, `formatDuration`, `readRunIdFromQuery`. Deterministic-zone pattern confirmed reusable — same layout as `manifest_explorer` (`view.html`, `<page>.js`, `<page>-mapping.js`, `<page>.css`, `AGENTS.md`). |
