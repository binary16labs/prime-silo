...
| ------------------------------------------- | ------- | ------------------------------------ | -------------------------------------------------------------------- | ---------- |
| **qwen2.5-coder-tuned** _(our house model)_ | 4.4 GB  | house-method coding + agent tool-use | **offload executor / candidate engine** (T4); `house/…` router alias | ✅ fits    |
...
_(eGPU column filled from the live check — see the verification table at the bottom.)_
...
- **Offload judge (ADR-004 gate)** → `google/gemma-3-4b` — fast, non-reasoning, and a _different
  family_ from the coder executor (anti-collusion, per ADR-004). Proven scoring 1.0 in T4. Never
...
- **Default planner / architect / SDLC** → `qwen/qwen3.5-9b` — the current default engine class
  ...
  the tuned model is _additive_ to, and the T4 no-regression baseline.
...
generation; embeddings: the `/v1/embeddings` endpoint). Latency is first-response _including
cold load_ (not steady-state speed), so it's a load+run proof, not a benchmark. Raw data:
