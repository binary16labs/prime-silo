...
Alpine.directive("destroy", (_el, { expression }, { evaluateLater, cleanup }) => {
...
Alpine.directive("inject", (el, { modifiers, expression }, { cleanup }) => {
...
Alpine.directive("move-to", (el, { expression, modifiers, value }, { evaluateLater }) => {
  const orderModifier = Array.isArray(modifiers) ? modifiers.find((m) => /^\d+$/.test(m)) : null;
...
Alpine.directive("every-second", (_el, { expression }, { evaluateLater, cleanup }) => {
...
Alpine.directive("every-minute", (_el, { expression }, { evaluateLater, cleanup }) => {
  const onTick = evaluateLater(expression);
...
Alpine.directive("every-hour", (_el, { expression }, { evaluateLater, cleanup }) => {
  const onTick = evaluateLater(expression);
