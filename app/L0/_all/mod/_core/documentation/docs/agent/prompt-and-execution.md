...
- `_core/onscreen_agent` also adds a small `current date and time` transient section every turn (today's date, local time + timezone, ISO timestamp, and the runtime mode) so the agent has live time awareness; it fails soft if the date cannot be read
