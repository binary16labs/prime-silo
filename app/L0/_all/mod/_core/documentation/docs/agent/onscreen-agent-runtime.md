...
The same module also adds a small `current date and time` transient section every turn (today's date, local time and timezone, ISO timestamp, and the `runtime-app`/`runtime-browser` mode) so the agent has live time awareness; it fails soft when the date cannot be read.
