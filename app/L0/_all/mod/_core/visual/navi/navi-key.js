...
class NaviKey extends HTMLElement {
  ...
  bindEvents() {
    ...
    const collapseArcs = () => {
      ...
      if (
        this._currentPetState === "happy" ||
        this._currentPetState === "code" ||
        this._currentPetState === "documents" ||
        this._currentPetState === "memory" ||
        this._currentPetState === "agents"
      ) {
  ...
  _scheduleEyeBlink() {
    ...
    this._blinkTimer = setTimeout(() => {
      if (this._isSleeping || this._expanded || this._currentPetState !== "idle") {
        this._scheduleEyeBlink();
        return;
      }
      const eyes = Array.from(this.querySelectorAll('.navi-key-dot[data-sym="B"]'));
      if (!eyes.length || !window.anime) {
        this._scheduleEyeBlink();
        return;
      }
      anime({
  ...
  _scheduleScanLine() {
    ...
    this._scanTimer = setTimeout(() => {
      if (this._isSleeping || this._expanded || !window.anime) {
        this._scheduleScanLine();
        return;
      }
      ...
      anime({
        ...
        fill: [
          ...
          {
            value: (el) => NaviKey.DOT_COLORS[el.getAttribute("data-sym")] || "#c4a882",
            duration: 350
          }
        ],
