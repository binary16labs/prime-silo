...
| Module                  | Owns                                                                         |
| ----------------------- | ---------------------------------------------------------------------------- |
| `runtime_client/`       | Fetch helper for `/api/runtime/*` calls; auto-injects `X-Benny-Agent-Scope` from active agent context. Also owns `saveView`/`loadView`/`listViews` (Phase D3), `signView`/`verifyView` (Phase F), and `pinView` (Phase F2). |
