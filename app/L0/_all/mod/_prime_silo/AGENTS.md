...
| `runs_explorer/`        | Second deterministic-zone shell page (Phase E expansion). Routed at `#/_prime_silo/runs_explorer`. Reads `/manifests/runs` + `/manifests/runs/<id>`, prefers `manifest_snapshot` for DAG fidelity, overlays execution status onto `dag.canvas` via `buildRunOverlay` + `mapManifestToDagData(options.runOverlay)`. Active runs float to the top of the dropdown. `?run_id=` deep-linking supported. |
...
- **Phase E expansion (runs_explorer)** — second deterministic-zone shell page shipped at [`runs_explorer/`](runs_explorer/AGENTS.md). Routed `#/_prime_silo/runs_explorer`. Reuses `manifest_explorer/manifest-mapping.js` unchanged via the `options.runOverlay` path. Operator opens the page and gets a dropdown of executed runs, sorted active-first; selecting a run renders its manifest DAG with execution-status colour overlay. The deterministic-zone pattern (same four-file module layout + AGENTS.md) is confirmed reusable. 46 tests pass.
