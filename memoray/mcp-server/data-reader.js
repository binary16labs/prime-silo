...
export class DataReader {
  ...
  static async getRecentSessions(limit = 10, projectPath = null, agent = null) {
    ...
    for (const id of sessionIds) {
      ...
      if (projectPath) {
        const sessionPath =
          session.metadata?.cwd || session.metadata?.projectPath || session.metadata?.project;
  ...
  static async getProjectStatsAndSkills(projectPath) {
    ...
    const SKILL_PATTERNS = [
      {
        keywords: ["database", "query", "sql", "schema"],
        skill: "Database querying & schema design"
      },
      { keywords: ["api", "rest", "endpoint", "fetch"], skill: "REST API integration" },
      {
        keywords: ["design", "ui", "layout", "css", "style", "responsive"],
        skill: "UI/UX design patterns"
      },
      {
        keywords: ["graph", "canvas", "render", "visuali"],
        skill: "Data visualisation & graph rendering"
      },
      ...
      {
        keywords: ["debug", "error", "fix", "bug", "patch"],
        skill: "Debugging & error resolution"
      },
      { keywords: ["refactor", "restructur", "reorgani", "clean"], skill: "Code refactoring" },
      {
        keywords: ["config", "setup", "install", "environment"],
      ...
      {
        keywords: ["session", "context", "memory", "history"],
        skill: "Context & session management"
      }
    ];
