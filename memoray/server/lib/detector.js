...
const GUIDES = {
  CLAUDE_SESSIONS_DIR:
    "Contains active Claude Code process PIDs. Created when Claude runs. If missing, try running a command like 'claude' in any directory to initialize.",
  CLAUDE_LOG_DIRS:
    "Directories where Claude Code saves session JSONL conversation histories. Normally created under ~/.claude/projects or local workspace AppData folder.",
  CLAUDE_WORKTREES_PATH:
    "JSON file created by Claude Code to track active git worktrees. Generated automatically when you create a new workspace in Claude.",
  CLAUDE_CONFIG_PATH:
    "Claude Desktop configuration file. Required to analyze active tools and MCP servers.",
  ANTIGRAVITY_BRAIN_DIRS:
    "Folder where Antigravity (Gemini IDE plugin) saves conversation trajectories. Ensure Antigravity pair programming has run at least once in your editor.",
  GEMINI_CONFIG_DIR: "Configuration folder for Gemini developer skills and global permissions.",
  OPENCODE_STORAGE_DIRS:
    "Folder where opencode saves its session/message/part JSON tree. Created under ~/.local/share/opencode/storage after you run opencode at least once.",
  OPENCODE_CONFIG_PATH:
    "opencode configuration file (providers, models, MCP servers, permissions). Normally ~/.config/opencode/opencode.json. Required to audit what opencode is allowed to do."
};
