        ...
        if (entry.tool_calls && entry.tool_calls.length > 0) {
          for (let ti = 0; ti < entry.tool_calls.length; ti++) {
            ...
            saveEntity({
              ...
              metadata: {
                toolName: tool.toolSummary || tool.name || "unknown",
    ...
    for (const sessionFolder of sessions) {
      ...
      if (shouldParseTranscript || isNew) {
        ...
        if (fs.existsSync(existingFile)) {
          try {
            existingSession = JSON.parse(fs.readFileSync(existingFile, "utf-8"));
          } catch {
            /* ignore */
          }
        }
        ...
        if (fs.existsSync(transcriptPath)) {
          try {
            sessionTimestamp = fs.statSync(transcriptPath).mtimeMs;
          } catch {}
        }
        ...
        saveEntity(
          {
            ...
            parent_id: null,
            children_ids: existingSession.children_ids || []
          },
          true
        ); // Force overwrite for session initialization
      ...
      // Parse artifacts
      totalArtifacts += parseArtifacts(
        sessionPath,
        sessionUUID,
        (index.antigravity_last_sync_timestamp || 0) - SYNC_BUFFER_MS
      );
    }
  }
