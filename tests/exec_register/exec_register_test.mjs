...
function sources() {
  return {
    g0Runs: [
      {
        ...
        machine: "t480",
        kind: "offload",
        valid_time: "2026-07-20T10:00:00Z",
        txn_time: "2026-07-20T10:05:00Z",
        hlc: "2026-07-20T10:05:00.000Z-0000-t480",
        model: { id: "house/qwen2.5-coder-tuned", hash: "sha256:aaa" },
        code_commit: "abc123",
        hw: { gpu: "gfx1200", vram_gib: 15.92 },
        knowledge_watermark: "sha256:kw1",
        hparams: {},
        events: [
          { event: "node_finished", duration_ms: 1900, tokens_in: 500, tokens_out: 200 },
          { event: "artifact_produced", content_hash: "sha256:out1" }
        ],
        quality: { judge: 1.0, gate_pass: true },
        cost_est: 0.0,
        authorship: "house"
      }
    ],
    trainJsons: [
      {
        model_id: "house/qwen2.5-coder-tuned",
        machine: "t480",
        valid_time: "2026-07-24T00:00:00Z",
        txn_time: "2026-07-24T02:00:00Z",
        agg_nll: 1.1253,
        base_agg_nll: 2.6195,
        code_commit: "abc123"
      }
...
test("Scenario: four logs fold into one schema", () => {
  ...
  for (const r of recs)
    for (const f of REQUIRED) assert.ok(f in r, `record missing ${f} (source ${r.source_log})`);
