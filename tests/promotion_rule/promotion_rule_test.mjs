...
test("Scenario: strict dominance decides", () => {
  const incumbent = mv(1.2, 0.8, 1.0, 2000);
  const better = mv(1.1, 0.85, 0.5, 1800); // better on EVERY metric
  const worse = mv(1.3, 0.75, 2.0, 2500); // worse on every metric
...
test("Scenario: a Pareto trade-off resolves by rule (eval-anchored), deterministically", () => {
  ...
  const evalGainCostLoss = mv(1.1, 0.8, 1.5, 2000);
  ...
  const evalLossCostGain = mv(1.3, 0.8, 0.5, 2000);
...
test("eval tie with mixed secondaries is deterministic and conservative (no eval gain → no swap)", () => {
  const incumbent = mv(1.2, 0.8, 1.0, 2000);
  const evalTieMixed = mv(1.2, 0.9, 2.0, 2000); // same eval, better agent_pass, worse cost
  ...
  const evalTieDominant = mv(1.2, 0.9, 0.5, 1800);
...
test("Scenario: a new eval slice does not rewrite history; comparisons use only shared slices", () => {
  ...
  const turn1 = { turn: 1, slices: { s1: 1.3, s2: 1.4 } };
  const turn2 = { turn: 2, slices: { s1: 1.2, s2: 1.35 } };
  // s3 is ADDED in turn 3 — it must not retroactively change turn1/turn2's scores.
  const turn3 = { turn: 3, slices: { s1: 1.1, s2: 1.3, s3: 0.9 } };
