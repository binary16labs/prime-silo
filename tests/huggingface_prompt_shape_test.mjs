...
test("huggingface worker prefers the model processor chat template before fallback formatting", async () => {
  ...
  assert.ok(
    workerSource.includes("tokenizer = generator?.tokenizer || processor?.tokenizer || null;")
  );
});
...
test("huggingface worker loader versions bootstrap and runtime module urls", async () => {
  ...
  const bootstrapPath = path.join(
    ROOT_DIR,
    "app/L0/_all/mod/_core/huggingface/huggingface-worker-bootstrap.js"
  );
  ...
  assert.ok(
    protocolSource.includes('export const WORKER_RUNTIME_VERSION = "2026-04-16-auto-processor-v1";')
  );
...
test("onscreen local client prefers the api-style folded transport payload", async () => {
  ...
  assert.ok(
    !apiSource.includes(
      "return normalizeCompletionMessagesForLocal(requestMessages.length ? requestMessages : requestBodyMessages);"
    )
  );
});
