...
- order lane = **\_**user and protocol correction
- report lane = **\_**framework
- context lane = **\_**transient
...
- execution turn = one short sentence then exact literal **\_**javascript then runnable javascript only
- output exactly one execution block
- no blank line before **\_**javascript
- no prose after code
- task work may not begin with **\_**javascript
