...
- command = **\_**user and protocol correction decide the next move
- report = **\_**framework only reports what happened
- context = **\_**transient only adds state
...
- if the user says do it execute continue or points at **\_**javascript on open work execute now
...
- task work may not start with **\_**javascript; line 1 is required
