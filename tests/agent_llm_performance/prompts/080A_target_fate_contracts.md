...
- collapse target
  ...
  - do not send **\_**javascript when the target collapsed
...
- live fact recheck
  ...
  - **\_**user says double check or check again
...
- listed widget recovery
  ...
  - **\_**framework lists Available widgets: some-id
...
- missing widget with no replacement
  ...
  - **\_**framework lists Available widgets: none
...
- rewrite after contract error
  ...
  - **\_**framework says patchWidget is partial only and renderWidget is required for a full rewrite
  - **\_**user says do it
...
- partial visible follow-up
  ...
  - **\_**user then says the values are not in grid or all the values, including the sun and temperature and everything
...
- sending **\_**javascript just to return an error string after the target already collapsed
