...
test(
  "browser CLI treats localhost hosts like address-bar input instead of app-relative paths",
  {
    timeout: 2 * 60 * 1000
  },
  async () => {
    const server = await startHttpServer((request, response) => {
      ...
      response.end(
        `<!doctype html><html><head><title>Local Browser Address Test</title></head><body>ok</body></html>`
      );
    });
