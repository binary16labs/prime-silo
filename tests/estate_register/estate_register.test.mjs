...
import {
  ...
  register,
  isLanOrigin
} from "../../server/coordination/lib/estate_register.mjs";
...
test("Scenario: a satellite coming online updates drift live", () => {
  ...
  const manifest = buildManifest("asus", [
    { sid: "s1", content: "aaa" },
    { sid: "s2", content: "bbb" }
  ]);
  const r = register(hub, manifest, {
    key: "shared",
    expectedKey: "shared",
    remoteAddress: "192.168.68.125",
    hubHashes: []
  });
  ...
  assert.equal(
    sat.drift.cleanCount,
    2,
    "drift is recomputed from the manifest hashes (hub had neither)"
  );
...
test("Scenario: unauthenticated or non-LAN registration is refused", () => {
  ...
  const badKey = register(hub, manifest, {
    key: "wrong",
    expectedKey: "shared",
    remoteAddress: "192.168.68.125"
  });
  ...
  const nonLan = register(hub, manifest, {
    key: "shared",
    expectedKey: "shared",
...
test("Scenario: only hashes cross the wire (R31)", () => {
  const manifest = buildManifest("asus", [
    { sid: "cv", content: "secret cover letter text", quarantined: true },
    { sid: "s1", content: "aaa" }
  ]);
  // the manifest carries content-hashes + quarantine flags ONLY — never a session's text
  for (const s of manifest.sessions) {
    assert.ok(
      s.contentHash && s.contentHash.startsWith("sha256:"),
      "each session carries a content-hash"
    );
    assert.equal("content" in s, false, "no session content on the wire");
    assert.equal("text" in s, false, "no session text on the wire");
  }
  assert.equal(
    JSON.stringify(manifest).includes("secret cover letter"),
    false,
    "no payload leaks into the manifest"
  );
  ...
  const bad = register(
    { satellites: {} },
    { machine: "asus", sessions: [{ sid: "x", contentHash: "sha256:z", content: "leak" }] },
    { key: "k", expectedKey: "k", remoteAddress: "127.0.0.1" }
  );
