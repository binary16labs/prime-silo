...
import { buildEstate, rebuildEstateFile, serializeEstate, isEstateSubjectId } from "../../server/coordination/lib/estate.mjs";
import { syncSource } from "../../server/coordination/lib/estate_sync.mjs";

function tmp() {
  const d = fs.mkdtempSync(path.join(os.tmpdir(), "n0-"));
  return { dir: d, kel: path.join(d, "kel.jsonl"), staging: path.join(d, "staging"), out: path.join(d, "estate.jsonl") };
}
...
test("Scenario: overlap dedupes by content", () => {
  ...
  syncSource(t.kel, t.staging, { machine: "t480", driveLabel: "D", driveRole: "replica", machineRole: "hub", sessions: [{ sid: "s1", content: "REPORT-BODY", project: "p" }] });
  syncSource(t.kel, t.staging, { machine: "t480", driveLabel: "F", driveRole: "source", machineRole: "hub", sessions: [{ sid: "s2", content: "REPORT-BODY", project: "p" }] });
  ...
  assert.deepEqual(est.sessions[sessions[0]].drives, ["t480:D", "t480:F"], "the one session is referenced by BOTH drives");
});

test("Scenario: delta-only re-sync", () => {
  const t = tmp();
  const src = { machine: "asus", driveLabel: "local", machineRole: "satellite", sessions: [{ sid: "a1", content: "SESSION-A" }] };
  ...
  assert.equal(estateEventCount(t.kel), eventsAfterFirst, "no new estate KEL event on an unchanged re-sync");
  assert.equal(lineCount(t.kel), linesAfterFirst, "an unchanged re-sync appends nothing to the log");
});

test("Scenario: changed content reprocesses", () => {
  const t = tmp();
  syncSource(t.kel, t.staging, { machine: "t480", driveLabel: "D", sessions: [{ sid: "s1", content: "V1" }, { sid: "keep", content: "KEEP" }] });
  const r = syncSource(t.kel, t.staging, { machine: "t480", driveLabel: "D", sessions: [{ sid: "s1", content: "V2" }, { sid: "keep", content: "KEEP" }] });
  ...
  assert.equal(Object.keys(est.sessions).length, 3, "changed content is a new entry; nothing is lost");
});

test("Scenario: the projection rebuilds from the log", () => {
  const t = tmp();
  syncSource(t.kel, t.staging, { machine: "t480", driveLabel: "F", driveRole: "source", machineRole: "hub", sessions: [{ sid: "s1", content: "A" }, { sid: "s2", content: "B" }] });
  syncSource(t.kel, t.staging, { machine: "asus", driveLabel: "local", machineRole: "satellite", sessions: [{ sid: "a1", content: "A" }, { sid: "q", content: "CV", quarantined: true }] });
