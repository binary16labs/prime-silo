...
async function runDesktopPackaging(platformKey, argv = process.argv.slice(2)) {
  ...
  // NOTE: the self-contained runtime bundle (Python + deps + Neo4j + JRE) is NO
  // LONGER packaged into the installer. Shipping ~600MB of executables/DLLs via
  // extraResources made electron-builder unreliable on Windows — Defender locks
  // freshly-written PE files mid-copy (EBUSY) and the build aborts before NSIS
  // (this is what broke v1.2.8). Instead the bundle is assembled + zipped as a
