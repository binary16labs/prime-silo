...
- L1 — portable CAS staging on D: (blobs + index + manifest) · author claude-opus · branch task/L1 @ 8724d30 · 2026-07-25 *(GATE GREEN: `node scripts/gates/l1.mjs` 5/5 — CAS de-dup (same content from 2 machines → 1 blob, both index records point to it), plug-and-play (openStaging resolves roots from the manifest, no per-machine config), self-describing index (machine/process/project/task_context/sid/links), offline staging emits a session_staged KEL event bound to the blob by content_hash, content-addressed path blobs/sha256/hh/hash. Reuses L0 kel.mjs for the event. Allowlist-clean (SPEC-knowledge-eventlog.md +Staging section, staging.mjs, tests/staging/, gates/l1.mjs); ~170 non-test lines < budget 500. Verifier: fresh non-author identity re-runs the gate from a clean checkout of task/L1, plus NEGATIVE probes — two different contents → two blobs (de-dup keys on hash, not sid); a mutation making casStore always-write → RED de-dup; then merge --no-ff task/L1. human-signed: owner directed "L1" 2026-07-25.)*
...
L2 L3 L5 L6 L7 L8 L9 L10 L11 L12 L13 L14 ·
