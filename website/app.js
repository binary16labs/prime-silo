...
function initHero() {
  ...
  track(
    animate("#hero-silo .hs-door, #hero-silo .hs-vent, #hero-silo .hs-legs", {
      opacity: [0, 1],
      duration: 500,
      delay: 720,
      ease: "out(2)"
  ...
  try {
    ...
    /* headline stays static (readable default) */
  }

  track(
    animate(".hero-content .section-kicker, .hero-content .section-sub, .hero-actions", {
      opacity: [0, 1],
      translateY: [16, 0],
  ...
  track(
    animate(".benny-hero .b-ear-right", {
      rotate: [0, 5],
      duration: 2200,
      alternate: true,
      loop: true,
...
function initDialChapter() {
  ...
  for (let k = 0; k < 6; k++) {
    ...
    tl.add(nodeAnim, { scale: [1, 1.3], duration: 300, ease: "outBack(2)" }, k * slot + 60)
      .add(
        `${nodeAnim} circle`,
        { fill: "#B94B2A", stroke: "#B94B2A", duration: 250 },
        k * slot + 60
      )
...
function initGovernanceChapter() {
  ...
  const sealParts = [".gov-seal-outer", ".gov-seal-inner", ".gov-seal-check"].map(
    (s) => createDrawable(`#governance-svg ${s}`)[0]
  );
  ...
  tl.add(
    ...
    "#gov-chain circle",
    { r: [0, 15], delay: stagger(130), duration: 380, ease: "outBack(2)" },
    3150
  );
  links.forEach((l, i) => {
...
function initLineageChapter() {
  ...
  const seal = [".ln-seal-outer", ".ln-seal-inner", ".ln-seal-check"].map(
    (s) => createDrawable(`#lineage-svg ${s}`)[0]
  );
  const quar = $$("#lineage-svg .ln-teleport-path, #lineage-svg .ln-quar-edge").map(
    (p) => createDrawable(p)[0]
  );
  ...
  ["#ln-cards", "#ln-graph", "#ln-book", "#ln-pdf", "#ln-seal", "#ln-gate"].forEach((s) =>
  ...
  // beat 0 — Session -> Card
  tl.add("#ln-cards", pop, 200).add(
    edges[0],
    { draw: "0 1", duration: 500, ease: "inOutSine" },
    300
  );
  // beat 1 — Card -> Graph
  tl.add("#ln-graph", pop, 1100).add(
    edges[1],
    { draw: "0 1", duration: 500, ease: "inOutSine" },
    1200
  );
  ...
  tl.add("#ln-book", pop, 2100)
    ...
    .add(
      "#ln-seal .ln-seal-pulse",
      { r: [30, 60], opacity: [0.8, 0], duration: 480, ease: "out(2)" },
      3160
    );
  ...
  tl.add("#ln-pdf", pop, 3300)
    ...
    .add(
      "#lineage-svg .ln-edge",
      { strokeWidth: [2, 3.2, 2], duration: 640, ease: "inOutSine" },
      3650
    )
  ...
  tl.add(box, { draw: "0 1", duration: 600, ease: "inOutSine" }, 4050)
    ...
    .add(
      quarItems[0],
      { opacity: [0, 1], translateY: [-10, 0], duration: 460, ease: "outBack(1.6)" },
      4720
    )
...
function initReveals() {
  ...
  $$(".reveal-group").forEach((group) =>
    targets.push({ el: group, items: $$(".reveal-item", group) })
  );
...
function initForgeChapter() {
  ...
  const branches = $$("#forge-svg .fg-branch, #forge-svg .fg-twig").map(
    (p) => createDrawable(p)[0]
  );
  ...
  utils.set([$("#fg-evidence"), $("#fg-tree"), $("#fg-gauge"), $("#fg-doc"), $("#fg-register")], {
    opacity: 0
  });
  ...
  tl.add(
    srcs,
    {
      ...
      delay: stagger(110),
      duration: 520,
      ease: spring({ stiffness: 130, damping: 13 })
    },
    200
  );
  ...
  tl.add(lanes, { draw: "0 1", delay: stagger(90), duration: 520, ease: "inOutSine" }, 1100)
    ...
    .add(
      "#fg-ev-count",
      { textContent: [0, 33967], modifier: utils.round(0), duration: 800 },
      1650
    );
  ...
  tl.add(ship, { draw: "0 1", duration: 520, ease: "inOutSine" }, 4400)
    ...
    .add(
      "#forge-svg .fg-thumb",
      { opacity: [0, 1], scale: [0.7, 1], delay: stagger(70), duration: 320, ease: "out(3)" },
      4850
    )
  ...
  ["output", "commit", "workspace"].forEach((mode, mi) => {
    ...
    rows.forEach((row, ri) => {
      ...
      tl.add(
        row,
        {
          translateX: slot[0] - slots[ri][0],
          translateY: slot[1] - slots[ri][1],
          duration: 420,
          ease: "inOutQuad"
        },
        t0 + 60 + ri * 30
...
function restoreForge() {
  $$("#forge-svg .fg-lane, #forge-svg .fg-branch, #forge-svg .fg-twig, #fg-ship, #fg-arc").forEach(
    (p) => {
  ...
  const ev = $("#fg-ev-count");
  if (ev) ev.textContent = "33967 links";
  const g = $("#fg-gates");
  if (g) g.textContent = "49/49";
  const w = $("#fg-words");
  if (w) w.textContent = "21111";
}
