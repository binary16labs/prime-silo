...
class IngestRequest(BaseModel):
    ...
    correlation_threshold: float = (
        0.82  # Similarity threshold for aggressive mode (raised from 0.70 — the 0.70 knee produced ~36× noise-tail edges)
    )
    ...
    do_ocr: bool = False  # Enable OCR during extraction
...
async def eval_triples(request: EvalTriplesRequest):
    ...
    return {
        ...
        "avg_confidence": (
            round(sum(t.confidence for t in triples) / len(triples), 2) if triples else 0
        ),
...
async def ingest_files(request: IngestRequest):
    ...
    try:
        ...
        task_manager.add_aer_entry(
            ...
            plan="1. Extract text via Docling 2. Chunk 3. Batch upsert to ChromaDB",
        )
    except Exception as e:
        ...
        for idx, file_path in enumerate(file_paths):
            try:
                ...
                task_manager.update_task(
                    ...
                    message=msg,
                    current_step=idx + 1,
                    metadata={"stage": "EXTRACTING", "current_file": file_path.name},
                )
                ...
                try:
                    track_aer(
                        ...
                        "rag_ingest",
                        request.workspace,
                        f"Extracting {file_path.name}",
                        "Docling engine started",
                    )
                except Exception:
                    pass
                text = extract_structured_text(file_path)
                print(
                    f"DEBUG: Extracted {len(text)} chars from {file_path.name}. Preview: {text[:100]}..."
                )
                ...
                if chunks:
                    try:
                        ...
                        logger.info(
                            f"Clearing old vectors for {file_path.name} to ensure fresh ingestion."
                        )
                        ...
                        for i in range(0, len(chunks), batch_size):
                            ...
                            batch_ids = [
                                f"{file_path.stem}_{run_id[-4:]}_{j}"
                            ...
                            collection.add(
                                documents=batch_chunks, metadatas=batch_metadatas, ids=batch_ids
                            )
                        ...
                        task_manager.add_aer_entry(
                            run_id, "Error", f"Indexing Failed {file_path.name}: {index_error}"
                        )
                ...
                if request.deep_synthesis:
                    try:
                        ...
                        track_aer(
                            ...
                            "rag_ingest",
                            request.workspace,
                            f"Deep Synthesis for {file_path.name}",
                            "Extracting triples via LLM Engine",
                        )
                        task_manager.update_task(
                            run_id,
                            metadata={"stage": "SYNTHESIZING", "current_file": file_path.name},
                        )
                        ...
                        sections = [
                            {"title": f"{file_path.name} Part {i+1}", "text": c}
                            for i, c in enumerate(synth_chunks)
                            if len(c) > 100
                        ]
                        ...
                        if triples:
                            ...
                            track_aer(
                                ...
                                "rag_ingest",
                                request.workspace,
                                f"Synthesis complete for {file_path.name}",
                ...
                task_manager.update_task(
                    run_id, progress=10 + int(80 * (idx + 1) / len(file_paths))
                )
        ...
        if request.deep_synthesis:
            try:
                ...
                track_aer(
                    ...
                    "rag_ingest",
                    request.workspace,
                    "Running Topological Clustering",
                    "Community detection started (LPA)",
                )
                ...
                track_aer(
                    ...
                    "rag_ingest",
                    request.workspace,
                    "Running Knowledge-to-Code Correlation",
                    "Cross-linking Concepts and Symbols",
                )
                ...
                avg_conf = (
                    sum_confidence / total_triples_extracted if total_triples_extracted > 0 else 0.0
                )
                task_manager.update_task(
                    run_id,
                    metadata={
...
async def pageindex_outline(source: str, workspace: str = "default"):
    ...
    from ..core.pageindex_builder import list_trees, load_tree
...
async def get_indexing_manifest(workspace: str = "default"):
    ...
    try:
        ...
        return {"workspace": workspace, "manifest": manifest, "total_indexed": len(indexed_sources)}
    except Exception as e:
...
async def query_rag(request: QueryRequest):
    ...
    try:
        ...
        results = collection.query(
            query_texts=[request.query], n_results=min(request.top_k, collection.count())
        )
...
async def chat_rag(request: QueryRequest):
    ...
    try:
        ...
        if request.mode == "graph_agent":
            ...
            from .studio_executor import topological_sort
            ...
            if not wf_def:
                track_workflow_fail(
                    ...
                    "chat_graph_agent",
                    request.workspace,
                    "System_GraphChatAgent definition missing",
                )
            ...
            from .studio_executor import StudioEdge, StudioNode
            ...
            if not request.active_nexus_id or request.active_nexus_id == "neural_nexus":
                track_workflow_fail(
                    run_id, "chat_graph_agent", request.workspace, "nexus_required interruption"
                )
                return {
                    "answer": "",
            ...
            chat_manifest = create_ephemeral_manifest(
                run_id,
                [
                    "query_graph",
                    "search_kb",
            ...
            # Initial track
            track_workflow_start(
                run_id, "chat_graph_agent", request.workspace, inputs=[request.query]
            )
            task_manager.create_task(request.workspace, "chat_graph_agent", task_id=run_id)

            from .studio_executor import execute_data_node, execute_llm_node, execute_trigger_node
            ...
            for node in sorted_nodes:
                try:
                    ...
                    if node.type == "trigger":
                        ...
                        output = await execute_llm_node(
                            node, context, request.workspace, run_id=run_id, agent_id=run_id
                        )
                    ...
                    if output.get("error"):
                        final_answer = (
                            f"Agent execution stopped at node '{node.id}': {output['error']}"
                        )
                        break
                except Exception as e:
            ...
            task_manager.update_task(
                run_id, status="completed" if final_answer else "failed", progress=100
            )
        ...
        if request.mode == "discovery_swarm":
            ...
            if not request.active_nexus_id or request.active_nexus_id == "neural_nexus":
                track_workflow_fail(
                    run_id, "discovery_swarm", request.workspace, "nexus_required interruption"
                )
                return {
                    "answer": "",
                    "status": "nexus_required",
            ...
            track_workflow_start(
                run_id, "discovery_swarm", request.workspace, inputs=[request.query]
            )
        ...
        if request.mode in ("adaptive", "agent"):
            ...
            track_workflow_start(
                run_id, "chat_adaptive_rag", request.workspace, inputs=[request.query]
            )
            ...
            if mentioned_sources:
                ...
                try:
                    gen_res = collection.query(
                        query_texts=[request.query], n_results=min(3, collection.count())
                    )
        ...
        context_text = ContextGuard.guard_string(
            context_text, profile.max_rag_context_chars, "rag_context"
        )
            ...
            async with httpx.AsyncClient(timeout=300.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "error" in data:
                        ...
                        track_workflow_fail(
                            run_id, "chat_semantic_rag", request.workspace, error_msg
                        )
                        raise HTTPException(
                            503, f"LLM Provider {provider_name} returned error: {error_msg}"
                        )
                    ...
                    answer = (
                        data.get("choices", [{}])[0]
                        .get("message", {})
                        .get("content", "No content in LLM response.")
                    )
                    ...
                    track_llm_call(
                        ...
                        parent_job_name="chat_semantic_rag",
                    )
                    track_workflow_complete(
                        run_id,
                        "chat_semantic_rag",
