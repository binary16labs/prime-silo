...
async def _run_and_record(goal: str, workspace: str, model: str | None, panels: int, run_id: str) -> None:
    try:
        await deep_produce(goal=goal, workspace=workspace, model=model, panel_count=panels, run_id=run_id)
...
async def start_deep_produce(req: DeepProduceRequest, background_tasks: BackgroundTasks) -> DeepProduceResponse:
    ...
    run_store.save_run(RunRecord(
        run_id=run_id, manifest_id=MANIFEST_ID, workspace=req.workspace,
