...
def topological_sort(nodes: List[StudioNode], edges: List[StudioEdge]) -> List[StudioNode]:
    ...
    if len(sorted_ids) != len(nodes):
        ...
        raise ValueError(
            f"Circular dependency or invalid references detected in graph involving nodes: {unsorted}"
        )
...
async def execute_trigger_node(node: StudioNode, message: str, context: Dict) -> Dict:
    ...
    return {"message": message or "No message provided", "trigger_type": trigger_type}
    ...
    if operation == "search":
        ...
        try:
            ...
            if collection.count() == 0:
                return {
                    "results": [],
                    "message": "Knowledge base empty — please ingest documents first",
                }
            ...
            return {"results": formatted, "context_text": context_text, "count": len(formatted)}
        ...
        try:
            ...
            md_content += (
                f"*Generated by Benny Studio — {datetime.now().strftime('%Y-%m-%d %H:%M')}*\n\n"
            )
            md_content += "---\n\n"
            md_content += content
        ...
        while current_step < max_steps:
            ...
            if response.status_code != 200:
                ...
                if "tools" in payload and (
                    response.status_code == 400 or "tool" in response.text.lower()
                ):
                    logging.warning(
                        f"Model {model_name} failed tool-calling payload. Retrying without tools."
                    )
                    payload.pop("tools")
                    response = await client.post(
                        chat_url, json=payload, headers={"Content-Type": "application/json"}
                    ...
                    if response.status_code != 200:
                        return {
                            "error": f"LLM error (after tool fallback): {response.status_code}",
                            "response": None,
                        }
                else:
                    return {
            ...
            if "choices" not in data or not data["choices"]:
                return {
                    "error": f"LLM unexpected response format: {json.dumps(data)}",
                    "response": None,
                }
            ...
            return {
                "response": format_combined_output(answer_body, reasoning),
...
async def _run_workflow_background(
    run_id: str, request: StudioExecuteRequest, sorted_nodes: List[StudioNode]
):
    ...
    manifest = create_ephemeral_manifest(
        run_id,
        [
            "query_graph",
            "search_kb",
    ...
    logging.info(
        f"[AUDIT] Background task started | run_id: {run_id} | workspace: {request.workspace} | nodes: {len(sorted_nodes)}"
    )
    ...
    try:
        emit_execution_checkpoint(
            run_id, request.workspace, "initialization", {"nodes_count": len(sorted_nodes)}
        )
        ...
        for node in sorted_nodes:
            _emit_execution_event(
                run_id,
                "node_started",
                {"nodeId": node.id, "nodeName": node.data.get("label", node.type)},
            )
            ...
            try:
                if node.type == "trigger":
                    ...
                    output = await execute_intervention_node(
                        node, context, request.workspace, run_id
                    )
                else:
                ...
                emit_node_execution_state(
                    run_id, request.workspace, node.id, status, outputs=output
                )
                _emit_execution_event(
                    run_id, "node_completed" if not has_error else "node_error", {"nodeId": node.id}
                )

                node_results.append(
                    NodeResult(node_id=node.id, node_type=node.type, status=status, output=output)
                )
                if has_error:
                ...
                emit_execution_failure(
                    run_id, request.workspace, ExecutionPhase.EXECUTION, e, node_id=node.id
                )
                overall_status = "failed"
                break

        _emit_execution_event(
            run_id, "workflow_completed" if overall_status == "completed" else "workflow_failed", {}
        )
...
async def execute_studio_workflow(request: StudioExecuteRequest):
    ...
    logging.info(
        f"[AUDIT] /api/workflows/execute called | workspace={request.workspace} | nodes={len(request.nodes)}"
    )
    ...
    if validation_error:
        logging.error(
            f"[AUDIT] Pre-flight validation failed | run_id: {run_id} | error: {validation_error}"
        )
        # Emit failure so it shows in governance despite not starting
        emit_execution_failure(
            run_id, request.workspace, ExecutionPhase.INITIALIZATION, ValueError(validation_error)
        )
...
async def execute_intervention_node(
    node: StudioNode, context: Dict, workspace: str, run_id: str
) -> Dict:
    """
    ...
    if is_breached:
        ...
        _emit_execution_event(
            ...
            "hitl_required",
            {
                ...
                "current_state_summary": "Breach detected in node output. Waiting for manual override.",
                "options": [
                    {
                        "label": "Approve (Override)",
                        "value": "approve",
                        "description": "Proceed despite breach",
                    },
                    {"label": "Reject", "value": "reject", "description": "Stop workflow"},
                    {
                        "label": "Edit & Resume",
                        "value": "edit",
                        "description": "Modify content then proceed",
                    },
                ],
            },
        )
...
async def execute_a2a_node(node: StudioNode, context: Dict, workspace: str) -> Dict:
    ...
    try:
        ...
        # Poll for completion
        final_task = await client.poll_until_complete(agent_url, task.id, max_wait=timeout)
