...
def compute_waves(
    tasks: List[Dict[str, Any]], dependency_graph: Dict[str, List[str]]
) -> List[List[str]]:
    """
    ...
    for task_id, deps in dependency_graph.items():
        ...
        for dep in deps:
            if dep not in task_ids:
                logger.warning(
                    "Dependency '%s' for task '%s' does not exist, ignoring", dep, task_id
                )
...
def detect_conflicts(wave: List[str], file_assignments: Dict[str, List[str]]) -> List[FileConflict]:
    """
...
def resolve_conflicts(
    waves: List[List[str]], file_assignments: Dict[str, List[str]]
) -> List[List[str]]:
    """
        ...
        for wave_idx, wave in enumerate(new_waves):
            ...
            if conflicts:
                ...
                for conflict in conflicts:
                    ...
                    if task_to_move not in new_waves[wave_idx]:
...
def generate_ascii_dag(
    tasks: List[Dict[str, Any]], dependency_graph: Dict[str, List[str]], waves: List[List[str]]
) -> str:
    """
...
def assign_models(
    ...
    local_reasoning = next(
        (
            k
            for k, v in model_registry.items()
            if "reasoning" in v.get("use_for", []) and v.get("cost_per_1k", 1.0) == 0
        ),
        None,
    )
    local_writing = next(
        (
            k
            for k, v in model_registry.items()
            if "content_generation" in v.get("use_for", []) and v.get("cost_per_1k", 1.0) == 0
        ),
        None,
    )
    local_fast = next(
        (
            k
            for k, v in model_registry.items()
            if "simple_tasks" in v.get("use_for", []) and v.get("cost_per_1k", 1.0) == 0
        ),
        None,
    )
    ...
    for task in tasks:
        ...
        if complexity == "high" or (words & reasoning_keywords):
            assignment[task["task_id"]] = local_reasoning or model_registry.get(
                "reasoning", {}
            ).get("model", "gpt-4-turbo")
        elif words & writing_keywords:
            assignment[task["task_id"]] = local_writing or model_registry.get("writing", {}).get(
                "model", "claude-3-sonnet-20240229"
            )
        elif words & fast_keywords:
            assignment[task["task_id"]] = local_fast or model_registry.get("fast", {}).get(
                "model", "gpt-3.5-turbo"
            )
        else:
