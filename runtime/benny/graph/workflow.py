...
def process_input(state: WorkflowState) -> dict:
    ...
    return {
        ...
        "context": {**state.get("context", {}), "processed": True},
    }
...
async def call_llm(state: WorkflowState) -> dict:
    ...
    try:
        ...
        return {"error": str(e), "current_node": "call_llm"}
...
def execute_tool(state: WorkflowState) -> dict:
    ...
    if not tool_fn:
        return {"error": f"Unknown tool: {tool_name}", "current_node": "execute_tool"}

    try:
        result = tool_fn(**tool_args)
        return {
            "tool_outputs": {**state.get("tool_outputs", {}), tool_name: result},
            "current_node": "execute_tool",
        }
    except Exception as e:
        return {"error": f"Tool execution failed: {str(e)}", "current_node": "execute_tool"}


def human_review(state: WorkflowState) -> dict:
    """Human-in-the-loop checkpoint - waits for approval"""
    return {"requires_approval": True, "current_node": "human_review"}
...
def should_use_tool(
    state: WorkflowState,
) -> Literal["execute_tool", "human_review", "format_output"]:
...
def build_workflow_graph(checkpointer=None) -> StateGraph:
    ...
    graph.add_conditional_edges(
        "execute_tool", after_tool, {"call_llm": "call_llm", "format_output": "format_output"}
    )

    # After human review
    graph.add_conditional_edges(
        "human_review", after_review, {"call_llm": "call_llm", "format_output": "format_output"}
    )
