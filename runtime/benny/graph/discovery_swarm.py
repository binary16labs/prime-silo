...
async def planner_node(state: DiscoveryState) -> Dict[str, Any]:
    ...
    if state["current_wave"] == 0 and not state.get("scout_waves"):
        ...
        task_manager.add_v2_telemetry(
            state["run_id"], "planner", {"message": "Mapping Layer 0 (Architecture)..."}
        )
        ...
        if not blueprint:
            return {
                "findings": [
                    "No grounded codebase nodes found in this Nexus. Blueprint is empty. Analysis aborted."
                ],
                "status": "completed",
            }
        ...
        task_manager.add_v2_telemetry(
            state["run_id"],
            "planner",
            {"message": f"Reasoning about relevance for: '{query[:30]}...'"},
        )

        blueprint_text = "\n".join(
            [f"- {b['path']} (Deps: {len(b.get('dependencies', []))})" for b in blueprint[:20]]
        )
...
OUTPUT: Return ONLY a JSON list of strings (file paths). Example: ["src/main.py", "api/server.py"]"""
        ...
        try:
            model_id = (
                f"{state['provider']}/{state['model']}" if state["provider"] else state["model"]
            )
        ...
        task_manager.add_v2_telemetry(
            state["run_id"],
            "planner",
            {"message": f"Selecting {len(scout_paths)} key files for scouting."},
        )

        return {"scout_waves": [scout_paths], "status": "exploring"}
...
async def scout_node(state: DiscoveryState) -> Dict[str, Any]:
    ...
    for path in wave_paths:
        task_manager.add_v2_telemetry(
            run_id, "scout", {"message": f"Peeking symbols in {path}", "layer": 1}
        )
...
Keep it to 2-3 sentences."""
        ...
        try:
            ...
            new_findings.append(
                f"**{path}**: Contains {len(symbols)} symbols including {symbol_list}."
            )
...
async def synthesize_node(state: DiscoveryState) -> Dict[str, Any]:
    ...
    task_manager.add_v2_telemetry(
        state["run_id"], "synthesizer", {"message": "Finalizing architectural report..."}
    )
...
"""
    ...
    try:
        ...
        answer = (
            f"I've explored the codebase but encountered an error during synthesis: {str(e)}\n\nFindings:\n"
            + "\n".join(state["findings"])
        )
