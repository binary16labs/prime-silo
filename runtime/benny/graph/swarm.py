...
def parse_json_safe(text: str) -> Tuple[Dict[str, Any], str]:
    ...
    cleaned = re.sub(
        r'("(?:\\["\\\/bfnrt]|\\u[0-9a-fA-F]{4}|[^"\\\x00-\x1f])*"\s*:\s*(?:"(?:\\["\\\/bfnrt]|\\u[0-9a-fA-F]{4}|[^"\\\x00-\x1f])*"|[^,\s{}[]]+))\s*(")',
        r"\1, \2",
        cleaned,
    )
    ...
    try:
        ...
        logger.debug(
            f"JSON repair failed. Original length: {len(text)}, Cleaned: {cleaned[:100]}..."
        )
        raise e
...
async def planner_node(state: SwarmState) -> Dict[str, Any]:
    ...
    if target_task:
        if depth >= max_depth:
            logger.warning(
                f"Reaching max planning depth ({max_depth}) for {target_task['task_id']}"
            )
            if target_pillar_id:
                for t in plan:
        ...
        task_manager.update_task(
            execution_id, status="running", message=f"Expanding pillar: {target_task['task_id']}..."
        )
        ...
        task_manager.update_task(
            execution_id, status="running", message="Generating initial strategy..."
        )
        task_manager.add_event(
            execution_id, "node_started", {"nodeId": "planner", "nodeName": "Hierarchical Planner"}
        )
        task_manager.add_v2_telemetry(
            execution_id,
            "planner",
            {
                "status": "thinking",
...
"""
            ...
            task_manager.add_event(
                execution_id,
                "node_completed",
                {"nodeId": "planner", "mode": mode, "count": len(new_tasks)},
            )
            return {
                "plan": updated_plan,
            ...
            if attempt == max_retries - 1:
                ...
                (diag_dir / "failed_planner_response.txt").write_text(
                    f"Error: {str(e)}\n\n{response_text}"
                )
...
def wave_scheduler_node(state: SwarmState) -> Dict[str, Any]:
    ...
    task_manager.add_event(
        execution_id, "node_started", {"nodeId": "wave_scheduler", "nodeName": "Wave Scheduler"}
    )
    ...
    try:
        ...
        file_assignments = {t["task_id"]: t.get("files_touched", []) for t in plan}
        ...
        topology = {
            ...
            "edges": [
                ...
                for target, srcs in dependency_graph.items()
                for src in srcs
            ],
        }
        ...
        task_manager.add_event(
            execution_id, "node_completed", {"nodeId": "wave_scheduler", "output": waves}
        )
...
def orchestrator_node(state: SwarmState) -> Command:
    ...
    if not waves:
        ...
        return Command(
            update={"status": "failed", "errors": ["No waves computed and no pillars to expand"]},
            goto=END,
        )
    ...
    return Command(update={"plan_approved": True, "status": "executing"}, goto="dispatcher")
...
async def executor_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    task_manager.add_event(
        execution_id,
        "node_started",
        {"nodeId": "executor", "nodeName": f"Executor: Task {task_id}"},
    )
    task_manager.add_v2_telemetry(
        execution_id,
        f"executor_{task_id}",
    ...
    try:
        ...
        if task.get("deterministic") and skill_hint:
            logger.info(
                "[DETERMINISTIC] Bypassing LLM for task %s using skill %s", task_id, skill_hint
            )
            skill_args = task.get("skill_args") or {}
            try:
                skill_result = await registry.execute_skill(
                    skill_hint, workspace_id, agent_id=execution_id, **skill_args
                )
...
Do this sparingly and only for significant knowledge gaps."""
        ...
        task_manager.add_event(
            execution_id, "node_completed", {"nodeId": "executor", "output": final_content}
        )
        ...
        return {
            "partial_results": [
                PartialResult(
                    task_id=task_id, content=None, error=str(e), execution_time_ms=execution_time
                )
            ]
        }
        ...
        if matches:
            ...
            if parent_task:
                ...
                if current_depth < 2:
                    ...
                    logger.warning(
                        f"Task {result['task_id']} reached recursion limit (depth {current_depth})"
                    )
...
def aggregator_node(state: SwarmState) -> Dict[str, Any]:
    ...
    task_manager.add_event(
        execution_id, "workflow_completed", {"artifact_path": str(artifact_path)}
    )
...
async def run_swarm_workflow(
    ...
    thread_config = {"configurable": {"thread_id": execution_id}, "max_concurrency": concurrency}
    ...
    try:
        ...
        outputs = [
            (
                f"reports/{os.path.basename(result.get('artifact_path', ''))}"
                if result.get("artifact_path")
                else f"reports/swarm_output_{execution_id}.md"
            )
        ]
        ...
        print(
            f"[LINEAGE] Completed tracking workflow {execution_id} ({execution_time_ms}ms) with outputs: {outputs}"
        )
    except Exception as e:
