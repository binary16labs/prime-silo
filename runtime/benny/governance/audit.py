...
def start_audit_service():
    ...
    if _worker_thread is None or not _worker_thread.is_alive():
        ...
        _worker_thread = threading.Thread(
            target=_audit_worker, name="BennyAuditWorker", daemon=True
        )
...
def emit_governance_event(
    event_type: str, data: Dict[str, Any], workspace_id: str = "global", mirror: bool = True
):
    """
    ...
    _log_queue.put(
        {"event_type": event_type, "payload": data, "workspace_id": workspace_id, "mirror": mirror}
    )
...
def _resolve_audit_log_path(workspace_id: str) -> Path:
    ...
    try:
        from ..core.workspace import (
            get_workspace_path,  # local import — avoid cycles at module load
        )
