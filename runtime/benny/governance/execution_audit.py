...
def emit_execution_failure(
    ...
    emit_governance_event(
        event_type="EXECUTION_FAILURE", data=failure_event, workspace_id=workspace_id, mirror=True
    )
...
def emit_execution_checkpoint(
    execution_id: str, workspace_id: str, checkpoint_name: str, data: Dict[str, Any]
):
...
def retrieve_execution_audit(
    ...
    if not audit_path.exists():
        return {"execution_id": execution_id, "status": "not_found", "events": []}
    ...
    try:
        ...
        if execution_events["failures"]:
            ...
            execution_events["first_error"] = {
                ...
                "message": execution_events["failures"][0]
                .get("data", {})
                .get("error", {})
                .get("message"),
            }
        ...
        return {"execution_id": execution_id, "status": "error", "error": str(e), "events": []}


def get_failed_nodes(execution_id: str, workspace_id: str) -> List[Dict[str, Any]]:
...
def generate_execution_report(execution_id: str, workspace_id: str) -> str:
    """Generate human-readable execution failure report."""
    audit = retrieve_execution_audit(
        execution_id, workspace_id, include_nodes=True, include_checkpoints=True
    )
    ...
    if audit.get("node_states"):
        failed_nodes = [
            n for n in audit["node_states"] if n.get("data", {}).get("status") == "failed"
        ]
        if failed_nodes:
            report_lines.extend(
                [
