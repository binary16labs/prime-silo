...
LINEAGE_HTTP_ENABLED = os.getenv("BENNY_LINEAGE_ENABLED", "").strip().lower() in (
    "1",
    "true",
    "yes",
    "on",
...
class BennyLineageClient:
    ...
    def _get_or_create_uuid(self, benny_id: Optional[str]) -> str:
        ...
        try:
            ...
            if benny_id not in self._run_id_map:
                ...
                logger.info(
                    "Mapped Benny ID '%s' to OpenLineage UUID '%s'",
                    benny_id,
                    self._run_id_map[benny_id],
                )
            return self._run_id_map[benny_id]

    def _create_run(
        self, benny_id: Optional[str] = None, facets: Optional[Dict[str, BaseFacet]] = None
    ) -> Run:
        ...
        if benny_id and "benny_context" not in actual_facets:
            actual_facets["benny_context"] = BennyContextFacet(
                benny_run_id=benny_id, workspace="default"  # Ideally passed, but defaulting for now
            )
    ...
    def _create_job(self, job_name: str, facets: Optional[Dict[str, BaseFacet]] = None) -> Job:
        ...
        return Job(namespace=self.namespace, name=job_name, facets=facets or {})
    ...
    def start_workflow(
        ...
        facets = {"benny_context": BennyContextFacet(benny_run_id=workflow_id, workspace=workspace)}
        ...
        try:
            ...
            emit_governance_event(
                event_type="LINEAGE_START_WORKFLOW", data=attr.asdict(event), workspace_id=workspace
            )
    ...
    def complete_workflow(
        ...
        if workflow_id in self._run_id_map:
            facets["benny_context"] = BennyContextFacet(
                benny_run_id=workflow_id, workspace=workspace
            )

        run = Run(runId=run.runId, facets=facets)
    ...
    def fail_workflow(
        self, workflow_id: str, workflow_name: str, workspace: str, error_message: str
    ) -> None:
        ...
        try:
            ...
            # Local Audit Log
            emit_governance_event(
                event_type="LINEAGE_FAIL_WORKFLOW", data=attr.asdict(event), workspace_id=workspace
            )
        except Exception as e:
    ...
    def emit_tool_execution(
        ...
        try:
            ...
            # Local Audit Log
            emit_governance_event(
                event_type="LINEAGE_TOOL_EXECUTION", data=attr.asdict(event), workspace_id="global"
            )
        except Exception as e:
    ...
    def emit_file_conversion(
        self, input_path: str, output_path: str, workspace: str, job_name: str = "pdf_to_markdown"
    ) -> str:
        ...
        input_dataset = InputDataset(namespace=self.namespace, name=input_path)

        output_dataset = OutputDataset(namespace=self.namespace, name=output_path)

        if hasattr(run, "facets"):
            run.facets["parent"] = ParentRunFacet(
                run={
                    "runId": str(uuid.uuid4())
                },  # Need a way to pass parent id to conversion if possible
    ...
    def emit_aer(
        ...
        try:
            ...
            # Local Audit Log
            emit_governance_event(
                event_type="LINEAGE_AER", data=attr.asdict(event), workspace_id=workspace
            )
        except Exception as e:
...
def track_workflow_fail(
    workflow_id: str, workflow_name: str, workspace: str, error_message: str
) -> None:
    """Track workflow failure - convenience function"""
    get_lineage_client().fail_workflow(workflow_id, workflow_name, workspace, error_message)
...
def track_file_conversion(
    input_path: str, output_path: str, workspace: str, job_name: str = "pdf_to_markdown"
) -> str:
    """Track a file format transformation - convenience function"""
    return get_lineage_client().emit_file_conversion(input_path, output_path, workspace, job_name)
...
def track_aer(
    ...
    get_lineage_client().emit_aer(run_id, job_name, workspace, intent, observation, inference, plan)
...
# =============================================================================
# ADR-001 — Agent authorship lineage
# =============================================================================
#
# The Prime-Silo shell fork constrains the in-browser agent to a single write
# subtree (``agent_sandbox/``). Every accepted sandbox write emits an
# ``AGENT_AUTHORSHIP`` audit event so the agent's authoring history is itself
# auditable — the Prime-Silo §A.7 alignment that says philosophical commitments
# must survive parallelism *and* fluidity.
#
# Phase A emits to the local rotating audit log (offline-safe). A follow-up
# will mirror these events into Marquez as a triple-lineage record with
# process=agent_authorship, skill=<agent_id>, data=<sandbox_path>.


def emit_agent_authorship(
    workspace_id: str,
    agent_id: str,
    sandbox_path: str,
    action: str,
    ...
    """Record an agent-authored write into the workspace audit log.

    Args:
        workspace_id: Workspace where the sandbox write happened.
        agent_id: Identifier of the authoring agent (recorded as ``skill``).
        sandbox_path: Workspace-relative path of the written artefact
            (e.g. ``agent_sandbox/views/exposure_review.aamp.view``).
        action: ``write`` | ``update`` | ``delete``.
        details: Optional structured context (bytes_written, content_hash,
            parent_run_id, etc.).
    """
    payload = {
