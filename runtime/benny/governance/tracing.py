...
def _get_status(code, msg: str = ""):
    ...
    if _Status is None:
        try:
            ...
            return None
    return (
        _Status(_StatusCode[code] if isinstance(code, str) else code, msg)
        if msg
        else _Status(_StatusCode[code] if isinstance(code, str) else code)
    )
...
def init_tracing(phoenix_url: str = PHOENIX_URL, service_name: str = SERVICE_NAME) -> bool:
    """
    ...
    try:
        ...
        register(endpoint=f"{phoenix_url}/v1/traces", project_name=service_name)
