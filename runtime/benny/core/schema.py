...
class WorkspaceManifest(BaseModel):
    ...
    model_thinking: Dict[str, str] = Field(default_factory=dict, description="Per-model reasoning toggle keyed by 'provider/model' id → 'on'|'off'. 'off' suppresses hidden chain-of-thought (call_model injects /no_think + enable_thinking:false). Absent/'on' = the model's default. Lets a reasoning model (e.g. gemma-4-12b-qat, Qwen3-8B-Hybrid) be run without thinking, while a model that needs thinking (qwen3.5-9b-FLM) is left on.")
