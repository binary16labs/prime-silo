...
class WorkspaceManifest(BaseModel):
    ...
    model_thinking: Dict[str, str] = Field(default_factory=dict, description="Per-model reasoning toggle keyed by 'provider/model' id → 'on'|'off'. Explicit operator override that wins over the profile default (except 'fragile' models, which are never suppressed). 'off' suppresses hidden chain-of-thought (call_model injects /no_think + enable_thinking:false). Absent = the profile/model default.")
    provider_profiles: Dict[str, str] = Field(default_factory=dict, description="Per-provider active model-profile name (provider → profile), overriding configs/model_profiles.json 'providers'. Selects which capability table (thinking-capable/fragile/none) governs auto-suppression of reasoning. Managed by the Agents screen.")
