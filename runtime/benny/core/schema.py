...
class SynthesisConfig(BaseModel):
    ...
    max_context_tokens: int = Field(
        default=600,
        description="Max input tokens per extraction chunk (~4 chars/token). Kept small so prompt(template ~260 tok)+section+output(768) fits small local context windows (FLM/NPU ~2048). Raise for large-context cloud models.",
    )
    min_section_chars: int = Field(
        default=50, description="Minimum chars for a section to be processed"
    )

    # Parallel extraction
    parallel_limit: int = Field(
        default=1, description="Max concurrent LLM calls for section extraction"
    )
    inference_delay: float = Field(
        default=0.5, description="Delay between calls to prevent thermal throttle"
    )
    ...
    max_conflict_triples: int = Field(
        default=30, description="Max triples sent to conflict detection"
    )
...
class LiveConfig(BaseModel):
    ...
    max_entities_per_run: int = Field(
        default=100, description="Hard cap on entities enriched per run"
    )
    auto_enrich_on_ingest: bool = Field(
        default=False, description="Automatically trigger enrichment when new files are ingested"
    )
...
class WorkspaceManifest(BaseModel):
    ...
    version: str = Field(
        default="1.0.0", description="Schema version for governance and migrations"
    )
    ...
    model_roles: Dict[str, str] = Field(
        ...
        description="Task-specific model assignments (chat, swarm, tts, stt, graph_synthesis)",
    )
    model_thinking: Dict[str, str] = Field(
        default_factory=dict,
        description="Per-model reasoning toggle keyed by 'provider/model' id → 'on'|'off'. Explicit operator override that wins over the profile default (except 'fragile' models, which are never suppressed). 'off' suppresses hidden chain-of-thought (call_model injects /no_think + enable_thinking:false). Absent = the profile/model default.",
    )
    provider_profiles: Dict[str, str] = Field(
        default_factory=dict,
        description="Per-provider active model-profile name (provider → profile), overriding configs/model_profiles.json 'providers'. Selects which capability table (thinking-capable/fragile/none) governs auto-suppression of reasoning. Managed by the Agents screen.",
    )
    embedding_provider: str = Field(default="local", description="Provider for vector embeddings")
    governance_tags: List[str] = Field(
        default_factory=list, description="Audit and compliance tags"
    )
    exclude_patterns: List[str] = Field(
        default_factory=lambda: ["**/node_modules/**", "**/dist/**", "**/build/**", "**/.venv/**"],
        description="Glob patterns for graph exclusion",
    )
    ...
    synthesis: SynthesisConfig = Field(
        default_factory=SynthesisConfig, description="Synthesis engine config"
    )

    # Live Mode
    live_mode: bool = Field(default=False, description="Enable external data enrichment")
    live_config: LiveConfig = Field(
        default_factory=LiveConfig, description="Live mode connector settings"
    )
...
class Task(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Custom task metrics and stage info"
    )
    aer_log: List[Dict[str, Any]] = Field(
        default_factory=list, description="Agent Execution Record narrative"
    )
    event_log: List[Dict[str, Any]] = Field(
        default_factory=list, description="Historical record of SSE events for this run"
    )
    topology: Optional[Dict[str, Any]] = Field(
        default=None, description="Graph nodes and edges for 3D visualization"
    )
