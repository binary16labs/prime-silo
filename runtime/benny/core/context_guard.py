...
class ContextGuard:
    ...
    def prepare_payload(messages: List[Dict[str, str]], model: str) -> List[Dict[str, str]]:
        ...
        logger.warning(
            f"[CONTEXT_GUARD] Payload size {total_size} exceeds budget {profile.max_total_chars}. Pruning history."
        )
        ...
        for msg in middle:
            ...
            content = ContextGuard.guard_string(
                msg.get("content", ""), limit, f"historic_{msg['role']}"
            )
