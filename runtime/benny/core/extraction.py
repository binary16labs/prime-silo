...
def extract_structured_text(
    file_path: Path, log_fn: Callable = print, use_docling: bool = True, do_ocr: bool = False
) -> str:
    """
    ...
    if not use_docling:
        log_fn(
            f"[INFO] Skipping Docling (use_docling=False), using basic extraction for {file_path.name}..."
        )
    ...
    try:
        ...
        pipeline_options.do_ocr = (
            do_ocr  # Faster and lighter if PDF has text; handles pages one-by-one.
        )
        pipeline_options.do_table_structure = (
            True  # Enabled to better extract headings and document structure.
        )
        ...
        if not markdown_content.strip():
            log_fn(
                f"[WARNING] Docling returned empty content for {file_path.name}. Falling back..."
            )
            return _basic_extract(file_path)
