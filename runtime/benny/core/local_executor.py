...
class BaseOpenAICompatibleExecutor(BaseLocalExecutor):
    ...
    async def generate(
        self, prompt: Content, system: Optional[str] = None, run_id: Optional[str] = None, **kwargs
    ) -> str:
        start_ts = time.time()
    ...
    async def stream(
        self, prompt: str, system: Optional[str] = None, run_id: Optional[str] = None, **kwargs
    ) -> AsyncIterator[str]:
        messages = []
        if system:
            messages.append({"role": "system", "content": system})
        ...
        self._emit_usage(
            run_id, prompt, "".join(full_content), int((time.time() - start_ts) * 1000)
        )
...
def resolve_executor(model_str: str) -> Optional[LocalExecutor]:
    ...
    if model_lower.startswith("lmstudio/"):
        ...
        return OpenAICompatibleExecutor(
            model_str.split("/", 1)[1], "lmstudio", "http://127.0.0.1:1234/v1"
        )

    if model_lower.startswith("fastflowlm/"):
        # Port 52625
        return OpenAICompatibleExecutor(
            model_str.split("/", 1)[1], "fastflowlm", "http://127.0.0.1:52625/v1"
        )
