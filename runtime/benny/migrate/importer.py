...
from ..core.manifest import MANIFEST_SCHEMA_VERSION, SwarmManifest
...
class MigrationReport:
    ...
    def add_transform(self, file_path: Path, action: str, details: str = ""):
        self.transforms.append({"file": str(file_path), "action": action, "details": details})
        ...
        return json.dumps(
            {
                ...
                "errors": self.errors,
                "metrics": {"total_files": len(self.transforms), "rewrites": self.count_rewrites},
            },
            indent=2,
        )
...
class MigrationEngine:
    ...
    ) -> Tuple[Optional[SwarmManifest], int]:
            ...
            try:
                ...
                with open(p, "r", encoding="utf-8") as f:
                    ...
                    if not isinstance(data, dict) or (
                        "plan" not in data and "requirement" not in data
                    ):
                        continue
                ...
                manifest, rewrites = self.migrate_manifest(
                    dest_p if not dry_run else p, dry_run=dry_run
                )
                if manifest:
                    report.add_transform(
                        rel_p, "UPGRADE", f"to v{MANIFEST_SCHEMA_VERSION} and signed"
                    )
