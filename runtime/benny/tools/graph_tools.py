...
@tool
def query_knowledge_graph(workspace: str = "default", active_nexus_id: Optional[str] = None) -> str:
    """
    ...
    try:
        ...
        lines = [
            f"📊 Knowledge Graph: {stats['concepts']} concepts, {stats['sources']} sources, {stats['relationships']} relationships"
        ]
...
def get_concept_neighbors(
    concept: str, workspace: str = "default", depth: int = 2, active_nexus_id: Optional[str] = None
) -> str:
    """
...
def add_knowledge_triple(
    subject: str, predicate: str, obj: str, source_name: str = "agent", workspace: str = "default"
) -> str:
    """
...
@tool
def find_structural_analogies(workspace: str = "default") -> str:
    """
    ...
    try:
        ...
        for edge in graph["edges"]:
            if edge["type"] == "ANALOGOUS_TO":
                ...
                tgt = node_map.get(edge["target"], "?")
                analogies.append(
                    f"  🔗 {src} ↔ {tgt}: {edge.get('description', '')} (Pattern: {edge.get('pattern', '')})"
                )
...
def search_similar_concepts(
    query: str, workspace: str = "default", top_k: int = 5, active_nexus_id: Optional[str] = None
) -> str:
    """
