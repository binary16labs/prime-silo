...
from .models import A2ATask, AgentCard, JsonRpcRequest, JsonRpcResponse, TaskState
            ...
            for path in ["/.well-known/agent.json", "/a2a/agent-card"]:
                try:
                    response = await client.get(
                        f"{agent_url.rstrip('/')}{path}", headers=self._headers()
                    )
            ...
            try:
                ...
                if response.status_code != 200:
                    raise A2AClientError(
                        f"Task send failed: {response.status_code} {response.text}"
                    )
