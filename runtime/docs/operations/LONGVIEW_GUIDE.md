...
| Map window fragments = `{"_error":"Engine protocol predict request failed: fetch failed"}` (400) | **LM host engine wedge.** Distinct from a timeout — the model reports `loaded` and embeddings still answer, but generation 400s on every predict. Trigger: a **second concurrent request** or an **aborted in-flight call** hitting the LM host (e.g. a health probe fired *during* a run, or killing a queued request). Fix: **eject + reload the model** on the LM host (no HTTP unload endpoint). Prevention: **one clean call at a time** — never run diagnostic probes against the LM host while a map/ingest is in flight; probe only between runs with the runner stopped. Timings measured under concurrency are inflated — do not trust them. |
