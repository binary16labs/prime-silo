...
| Ingest loops on the same batch; task_registry shows "No model loaded: <A>" then "No model loaded: <B>" | **Engine swap-thrash (fixed v1.12.3 / A8).** Default-role calls (clustering/correlation) used to fall through to lemonade's first catalog model while graph_synthesis pinned qwen — two engines evicting each other per call on one NPU. Now the ingest request's `model` pins the whole run (run affinity), auto-detect prefers the currently-loaded model, and a last-resort catalog pick logs a "model roulette" WARNING. Belt-and-braces: set `BENNY_DEFAULT_MODEL=lemonade/qwen3.5-9b-FLM` in the runtime's environment to pin every role globally. |
| Batch burns the full 4 h timeout while the server task is dead | **Task-stall detection (v1.12.3).** The client now fails a batch after `LONGVIEW_INGEST_STALL_MS` (default 30 min) without task-record progress, and on any failed deep-synthesis batch it reconciles per-file against `.benny/wiki/` evidence so the retry skips completed cards instead of re-ingesting the batch. |
| Task frozen at SYNTHESIZING (n/40), updated_at not advancing, lemonade idle | **Server-side missing deadline (fixed v1.12.6 / A9).** Each file's synthesis now runs under `BENNY_SYNTH_FILE_TIMEOUT` (default 1200s), graph writes under `BENNY_GRAPH_WRITE_TIMEOUT` (300s), embeddings under `BENNY_EMBED_TIMEOUT` (120s, retries pinned). A hung step fails that FILE with a named error; the task keeps advancing. Client-side, a stalled task stops the phase (A8.2) — restart the runtime, rerun; startup reconcile resumes. |
| Map window fragments = `{"_error":"The operation was aborted due to timeout"}` | The window's LLM call ran past `LONGVIEW_LLM_TIMEOUT_MS`. On a reasoning model (gemma-4-12b) a full **24k-char** window's prefill + reasoning preamble can exceed 200s. Fix: **shrink `LONGVIEW_WINDOW_CHARS`** (24000→12000 halves the prefill so each call finishes reliably) **and/or raise `LONGVIEW_LLM_TIMEOUT_MS`** (e.g. 420000). Smaller windows mean more calls per session (the reasoning tax is paid per call), so it trades throughput for completeness. Purge the affected cards + window dirs so they re-map cleanly. |
| Map window fragments = `{"_error":"Engine protocol predict request failed: fetch failed"}` (400) | **LM host engine wedge.** Distinct from a timeout — the model reports `loaded` and embeddings still answer, but generation 400s on every predict. Triggers: a **second concurrent request** or an **aborted in-flight call**, but it also happens **spontaneously under sustained load** (independent of reasoning on/off). Since v1.15.2 the map **auto-halts** on it (`isEngineWedge`): the failed window is left uncached (resume retries it), the phase stops, and it exits non-zero so `build_v2.sh` stops before graph/enrich — damage is capped at one session, not the whole queue. Fix: **eject + reload the model**, then rerun (resume re-maps the unwritten sessions). Prevention for the concurrency trigger: **one clean call at a time** — no probes against the LM host during a run. For the spontaneous trigger, read LM Studio's OWN logs at the crash (OOM/KV-cache → lower context or GPU layers; GPU/driver error → host-side; a poison input re-crashes the same session on resume). |
| Map absurdly slow on a reasoning model (gemma-4-12b) | ~78% of every call is a reasoning preamble that adds nothing to extraction. Set **`LONGVIEW_REASONING_EFFORT=none`** (v1.15.2) — LM Studio's on/off reasoning control, sent per request as `reasoning_effort:"none"`. Verified ~4× faster (160s/window → 42s) with no quality loss. Note: the prompt can't suppress it, and `enable_thinking:false` / `reasoning_effort:"low"` are silently ignored. |
