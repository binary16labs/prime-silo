...
- longview builds the knowledge graph two ways over the SAME schema (`Source`/`Concept`/`RELATES_TO`/`SOURCED_FROM`): the `model` phase re-extracts triples with a second LLM pass (deep_synthesis, ~60-120s/card), while the `graph` phase (longview_v2) derives them deterministically from each card's own `concepts[]`/`applications[]`/`capabilities[]`/`skills_observed[]` arrays with no model call (~0.4s/card) — prefer `graph` since the `map` phase already distilled those entities; run `enrich` afterwards to merge duplicate concepts across cards into shared hubs. Server route `POST /rag/graph-upsert`; pure mapper `scripts/longview/lib/card_triples.mjs`; live earned-ETA in `<workspace>/longview/progress.json`
- the longview generation model is env/profile-driven (`BENNY_DEFAULT_MODEL` / `LONGVIEW_MODEL`), never hardcoded — the manifest carries only a preconfigured default so provider/model profiles stay swappable for eval comparison; a reasoning model (e.g. `lmstudio/google/gemma-4-12b`) needs its per-call token budget to clear the reasoning preamble or content returns empty, and LM Studio requires the full org/model id plus `response_format` `json_schema|text` (never `json_object`)
