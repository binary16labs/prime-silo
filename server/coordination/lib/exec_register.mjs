...
function record({
  ...
  return {
    ...
    config,
    metrics,
    lineage: lineage ?? { inputs: [], outputs: [] },
    authorship,
    source_log,
...
export function fromG0Run(run) {
  ...
  return record({
    exec_id: detId(`g0:${run.run_id}`),
    kind: run.kind ?? "run",
    run_id: run.run_id,
    valid_time: run.valid_time,
    txn_time: run.txn_time,
    hlc: run.hlc,
    machine: run.machine,
    config: {
      model_id: run.model?.id ?? null,
      model_hash: run.model?.hash ?? null,
      code_commit: run.code_commit ?? null,
      hw: run.hw ?? {},
      knowledge_watermark: run.knowledge_watermark ?? null,
      hparams: run.hparams ?? {}
    },
    metrics: {
      wall_ms,
      tokens_in,
      tokens_out,
      cost_est: run.cost_est ?? 0,
      quality: run.quality ?? {}
    },
    lineage: { inputs, outputs },
    authorship: run.authorship ?? null,
    source_log: "g0"
  });
}
...
export function fromTrainJson(j) {
  return record({
    exec_id: detId(`train:${j.model_id}:${j.txn_time}`),
    kind: "train",
    valid_time: j.valid_time,
    txn_time: j.txn_time,
    hlc: j.hlc ?? `${j.txn_time}-0000-${j.machine ?? "t480"}`,
    machine: j.machine ?? "t480",
    config: {
      model_id: j.model_id ?? null,
      model_hash: j.model_hash ?? null,
      code_commit: j.code_commit ?? null,
      hw: j.hw ?? {},
      knowledge_watermark: j.knowledge_watermark ?? null,
      hparams: j.hparams ?? {}
    },
    metrics: {
      wall_ms: j.wall_ms ?? null,
      tokens_in: null,
      tokens_out: null,
      cost_est: j.cost_est ?? 0,
      quality: { eval_nll: j.agg_nll ?? null, base_nll: j.base_agg_nll ?? null }
    },
    lineage: { inputs: [], outputs: [] },
    authorship: j.authorship ?? null,
    source_log: "train-json"
  });
}

export function fromLongviewLedger(e) {
  return record({
    exec_id: detId(`longview:${e.phase}:${e.txn_time}`),
    kind: "synthesis",
    valid_time: e.valid_time,
    txn_time: e.txn_time,
    hlc: e.hlc ?? `${e.txn_time}-0000-${e.machine ?? "t480"}`,
    machine: e.machine ?? "t480",
    config: {
      model_id: e.model_id ?? null,
      model_hash: null,
      code_commit: e.code_commit ?? null,
      hw: {},
      knowledge_watermark: e.knowledge_watermark ?? null,
      hparams: {}
    },
    metrics: {
      wall_ms: e.wall_ms ?? null,
      tokens_in: null,
      tokens_out: null,
      cost_est: e.cost_est ?? 0,
      quality: { outcome: e.outcome ?? null }
    },
    lineage: { inputs: [], outputs: [] },
    source_log: "longview"
  });
...
export function fromCoordEvent(e) {
  return record({
    exec_id: detId(`coord:${e.id}`),
    kind: "agent",
    run_id: e.run_id ?? null,
    valid_time: e.ts,
    txn_time: e.ts,
    hlc: `${e.ts}-0000-${e.machine ?? "unknown"}`,
    ...
    metrics: {
      ...
      tokens_out: null,
      cost_est: 0,
      quality: { task_id: e.task_id, event: e.type }
    },
    lineage: { inputs: [], outputs: [] },
    authorship: e.agent === "human" ? "human" : null,
    source_log: "coordination"
  });
}
...
export function compareExecutions(records, { kind, delta = false } = {}) {
  ...
  return {
    a: {
      ...
      cost: a?.metrics?.cost_est,
      quality: q(a)
    },
    b: {
      machine: b?.machine,
      model: b?.config?.model_id,
      cost: b?.metrics?.cost_est,
      quality: q(b)
    },
