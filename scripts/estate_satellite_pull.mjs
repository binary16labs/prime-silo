...
const LANDING = opt("--landing", process.env.ESTATE_SATELLITE_LANDING || "D:/asus_ingest").replace(
  /\\/g,
  "/"
);
...
function quarantineSelftest(q) {
  ...
  return {
    ...
    why: !allBlocked
      ? "A QUARANTINED SID IN UUID FORM WAS NOT BLOCKED"
      : control
        ? "filter blocks everything — too broad to be meaningful"
...
function* sessionFiles(root) {
  ...
  for (const d of dirs) {
    if (!d.isDirectory()) {
      if (d.name.endsWith(".jsonl")) yield { project: null, file: path.join(base, d.name) };
      continue;
    }
    let files = [];
    try {
      files = fs.readdirSync(path.join(base, d.name));
    } catch {
      continue;
    }
    for (const f of files)
      if (f.endsWith(".jsonl")) yield { project: d.name, file: path.join(base, d.name, f) };
  }
}
...
function fetchFromShare(from, landing, q) {
  ...
  for (const { project, file } of sessionFiles(from)) {
    ...
    if (fs.existsSync(dest) && sha256(fs.readFileSync(dest)) === sha256(src)) {
      skipped++;
      continue;
    }
...
if (!invokedDirectly) {
  ...
  if (flag("--probe")) {
    const share = FROM
      ? probe(FROM)
      : { root: null, reachable: null, why: "no --from share configured" };
      ...
      JSON.stringify(
        {
          ...
          landing: {
            ...land,
            note: "hub-local storage — its presence says nothing about the satellite"
          },
  ...
  if (flag("--fetch")) {
    if (!FROM) {
      console.error("--fetch needs --from <share path> (or ESTATE_SATELLITE_SHARE)");
      process.exit(2);
    }
    const r = fetchFromShare(FROM, LANDING, q);
    if (!r.reachable) {
      console.error(`satellite ${SATELLITE} UNREACHABLE at ${FROM}: ${r.why}`);
      process.exit(3);
    }
    console.log(`fetch ${FROM} -> ${LANDING}`);
    console.log(
      `  copied ${r.copied} · unchanged ${r.skipped} · quarantine-withheld ${r.quarantine_withheld}`
    );
  }
  ...
  if (!hub.ok) {
    ...
    console.error(
      "With no baseline, every satellite session looks new and a sync would re-stage the corpus."
    );
  ...
  if (!sig) {
    ...
    console.log(
      `  hub holds            ${hub.hashes.size} session content-hashes${hub.ok ? "" : ` (KEL unreadable: ${hub.why})`}`
    );
    console.log(`  satellite scanned    ${delta.total}`);
    console.log(`  already in hub       ${delta.overlap}  (content-hash overlap, excluded)`);
    console.log(
      `  quarantine withheld  ${sat.quarantined + delta.quarantined.count}  (counted, never named)`
    );
    ...
    if (hub.hashes.size > 0 && delta.overlap === 0 && delta.total > 0) {
      ...
      console.log(
        `    same grain before signing — a grain mismatch inflates the delta to "everything".`
      );
    }
    console.log(
  ...
  if (q.size > 0) {
    ...
    if (!covered) {
      ...
      console.error(
        `matches NO session on ${SATELLITE} — the filter has nothing to compare against.`
      );
      console.error(
        `quarantine: ${[...shapes].join(", ")} · satellite: ${idShape(sat.sessions[0]?.sid || "")}`
      );
      process.exit(5);
    }
  ...
  if (proposal.count === 0) {
    console.log(
      `nothing to sync — the hub already holds every clean session on ${SATELLITE}. No-op.`
    );
    process.exit(0);
  }
  ...
  console.log(
    `  sessions staged     ${result.syncResult.sessionsNew} new · ${result.syncResult.sessionsSkipped} already recorded`
  );
  console.log(
    `  blobs               ${result.syncResult.stored} stored · ${result.syncResult.deduped} deduped`
  );
