...
const secsBetween = (a, b) => {
  const t1 = Date.parse(a),
    t2 = Date.parse(b);
  return Number.isFinite(t1) && Number.isFinite(t2) && t2 >= t1
    ? Math.round((t2 - t1) / 100) / 10
    : null;
};
    ...
    const edges = (plan.edges || [])
      .map((e) =>
        Array.isArray(e)
          ...
          : [String(e.source || e.from || ""), String(e.target || e.to || "")]
      )
      .filter((p) => p[0] && p[1]);
    contracts[m.id || f] = {
      id: m.id || f,
...
function foldRunRecords(execs, bennyHome, workspace, contracts) {
  ...
  let entries = [];
  try {
    entries = fs.readdirSync(runsDir, { withFileTypes: true }).filter((d) => d.isDirectory());
  } catch {
    return;
  }
  for (const d of entries) {
    ...
    try {
      ...
      /* not every run folder carries its manifest */
    }
    try {
  ...
  for (const line of lines) {
    ...
    switch (e.event_type) {
      case "TASK_METADATA_UPDATE": {
        const ex = touch(
          d.task_id,
          classify(d.task_id, { ...d, producer: (d.metadata || {}).producer })
        );
        if (!ex) break;
        used++;
        if (d.status) ex.status = d.status;
        if ((d.metadata || {}).producer) ex.producer = d.metadata.producer;
        if (d.type && ex.type === "unknown") {
          ex.type = d.type;
          ex.type_label = EXEC_TYPES[d.type] || d.type;
        }
      ...
      case "NODE_EXECUTION_STATE": {
        ...
        if (!ex) break;
        used++;
        const existing = ex.processes.find(
          (p) => p.node_id === d.node_id && p.source === "node_state"
        );
        const rec = existing || {
          node_id: d.node_id,
          wave: null,
          status: d.status,
          started: iso(d.timestamp),
          duration_s: null,
          error: null,
          lineage_ref: null,
          integrity_sha256: null,
        ...
        if (typeof d.duration_ms === "number" && d.duration_ms > 0)
          rec.duration_s = Math.round(d.duration_ms) / 1000;
      ...
      case "ENRICH_TASK_FAILED": {
        ...
        if (rec) {
          rec.status = "failed";
          rec.error = String(d.error || "").slice(0, 300);
        } else
          ex.processes.push({
            node_id: d.task_id,
            wave: d.wave ?? null,
            status: "failed",
            started: iso(d.timestamp),
            duration_s: d.elapsed_s ?? null,
            error: String(d.error || "").slice(0, 300),
            lineage_ref: d.lineage_ref || null,
            integrity_sha256: d._sha256 || null,
            source: "ledger"
          });
      ...
      case "LINEAGE_TOOL_EXECUTION": {
        ...
        for (const i of d.inputs || [])
          if (i && i.name && !ex.inputs.includes(i.name)) ex.inputs.push(i.name);
        for (const o of d.outputs || [])
          if (o && o.name && !ex.outputs.includes(o.name)) ex.outputs.push(o.name);
        break;
      }
      default:
        break;
...
export function bindContract(ex, contracts) {
  ...
  if (ex.template_id && contracts[ex.template_id]) {
    return {
      contract_file: contracts[ex.template_id].file,
      contract_id: ex.template_id,
      method: "declared_template_id",
      confidence: 1.0
    };
  }
  ...
  const stem = String(ex.execution_id || "")
    .replace(/-?\d{8,}$/, "")
    .replace(/-$/, "");
  if (stem.length >= 6) {
    for (const c of Object.values(contracts)) {
  ...
  if (best && best.score >= 0.6) {
    return {
      contract_file: best.c.file,
      contract_id: best.c.id,
      method: "task_set_evidence",
      confidence: Math.round(best.score * 100) / 100,
      matched_tasks: best.hit,
      declared_tasks: best.declared
    };
  }
  ...
  return {
    contract_file: null,
    contract_id: ex.contract_id || null,
    method: "unbound",
    confidence: 0
  };
}
...
export function buildRegister({ repoRoot, bennyHome, workspace }) {
  ...
  const executions = [...execs.values()]
    .map((ex) => {
      ...
      ex.contract_coverage =
        c && c.declared_task_count
          ? {
              declared: c.declared_task_count,
              executed: ex.process_count,
  ...
  for (const e of executions) {
    for (const i of e.inputs)
      (datasets[i] = datasets[i] || { produced_by: [], consumed_by: [] }).consumed_by.push(
        e.execution_id
      );
    for (const o of e.outputs)
      (datasets[o] = datasets[o] || { produced_by: [], consumed_by: [] }).produced_by.push(
        e.execution_id
      );
  }
  ...
  return {
    ...
    sources: {
      governance_log: {
        lines: ledger.lines || 0,
        events_used: ledger.used || 0,
...
if (process.argv[1] && path.resolve(process.argv[1]).endsWith("exec_register.mjs")) {
  ...
  const outPath = arg(
    "--out",
    path.join(bennyHome, "workspaces", workspace, "longview", "lineage", "execution_register.json")
  );
  ...
  if (process.argv.includes("--json")) console.log(JSON.stringify(reg.totals, null, 1));
    ...
    console.log(
      `  processes         : ${reg.totals.processes} (${reg.totals.failed_processes} failed)`
    );
    ...
    console.log(
      `  integrity         : ${reg.totals.integrity_hashed_events} hashed events, ${reg.totals.integrity_hashed_records} hashed records`
    );
  }
}
