...
if (!/estateApi\.tryHandle\(req, res\)/.test(appjs))
  fail("app.js does not mount estateApi.tryHandle");
if (
  !/if \(estateApi\.tryHandle\(req, res\)\) return undefined;[\s\S]*return requestHandler/.test(
    appjs
  )
)
  fail("estate mount must run BEFORE the generic requestHandler (fall-through preserved)");

const t = spawnSync(process.execPath, ["--test", "tests/estate_api/estate_api.test.mjs"], {
  cwd: ROOT,
  stdio: "inherit"
});
if (t.status !== 0) {
  console.log("[n2] GATE FAILED (scenarios)");
  process.exit(1);
}
