...
try {
  ...
  /* no .env available — fall back to ambient env + defaults */
}
...
if (only) {
  const t = reg.types.find(
    (x) => x.id === only || x.label.toLowerCase().includes(only.toLowerCase())
  );
  if (!t) {
    console.error(`unknown workflow type: ${only}`);
    process.exit(2);
  }
  ...
  for (const v of t.versions) {
    ...
    const extra = [
      ...
      v.formats?.length && `[${v.formats.join(",")}]`,
      v.quality != null && `q=${v.quality}`,
      v.agg_nll != null && `nll=${v.agg_nll}`
    ]
      .filter(Boolean)
      .join("  ");
...
for (const t of reg.types) {
  ...
  console.log(
    short(t.label, 23).padEnd(24) +
      t.kind.padEnd(12) +
      latest.padEnd(30) +
      day(t.latest?.generated).padEnd(12) +
      String(t.count)
  );
}
console.log(
