...
export function listWorkspaces({ includePrivate = false } = {}) {
  ...
  let names = [];
  try {
    names = fs.readdirSync(WORKSPACES_ROOT);
  } catch {
    return out;
  }
  for (const name of names) {
    ...
    if (isPrivate && !includePrivate) {
      out.push({ name, root, private: true, scanned: false });
      continue;
    }
...
function discoverTogaf(ws) {
  ...
  for (const f of ls(dir)) {
    ...
    if (v) {
      v.rank = Number(m[1]);
      v.workspace = ws.name;
      v.state = exists(path.join(dir, `togaf_epic_v${m[1]}_state.json`));
      versions.push(v);
    }
  }
  return {
    ...
    generator: "scripts/longview/ (togaf epic) — see togaf-sad-v5-flow",
    versions
  };
}
...
function discoverBook(ws) {
  ...
  for (const name of ls(itDir)) {
    ...
    if (!main) continue;
    const v = fileVersion(main, {
      version: name,
      label: `${name} — ${ws.name}/iterations/${name}`
    });
    if (!v) continue;
  ...
  return {
    id: "ai_vampire_book",
    label: "The AI Vampire (book)",
    kind: "document",
    produces: "Long-form book + coverage report",
    generator: "scripts/longview/longview.mjs opus (LONGVIEW_OPUS_DIR per iteration)",
    versions
  };
}
...
function discoverLongviewReports(ws) {
  ...
  for (const [f, label] of wanted) {
    const v = fileVersion(path.join(dir, f), {
  ...
  return {
    id: "longview_reports",
    label: "LONGVIEW reports",
    kind: "report",
    produces: "Portfolio / themes / timeline / PRD",
    generator: "scripts/longview/longview.mjs reduce",
    versions
  };
}
...
function discoverCards(ws) {
  ...
  let newest = null,
    model = null;
  for (const f of cards.slice(-60)) {
    // sample the tail for freshness/model
    const m = readJson(path.join(cardsDir, f.replace(/\.json$/, ".meta.json")));
    if (m?.ts && (!newest || m.ts > newest)) {
      newest = m.ts;
      model = m.model || model;
    }
  }
  ...
  return {
    ...
    versions: [
      {
        version: `${ws.name}: ${cards.length} cards`,
        label: `${cards.length} cards (${ws.name})`,
        path: cardsDir,
        generated: newest || null,
        model,
        workspace: ws.name,
        rank: cards.length,
...
function discoverLadder(repoRoot) {
  ...
  for (const f of ls(dir)) {
    ...
    versions.push({
      version: d.model,
      label: `${d.model}${f.includes("rung2") ? " (rung2)" : ""}`,
      path: path.join(dir, f),
      generated: st ? st.mtime.toISOString() : null,
      quality: d.quality_score ?? null,
      wall_seconds: d.wall_seconds ?? null
    });
  }
  versions.sort(byRecency);
  return {
    id: "model_ladder",
    label: "Model ladder (bench)",
    kind: "evaluation",
    produces: "Per-model quality + speed on the LONGVIEW map primitive",
    generator: "docs/bench/rung1/rung2_bench.mjs (force LONGVIEW_LLM_BASE_URL=localhost)",
    versions
  };
}
...
function discoverDistill(repoRoot) {
  ...
  for (const [sub, label] of [
    ["out_p5", "P5 — LONGVIEW fragment"],
    ["out_ta", "EP-A — tool use"]
  ]) {
    ...
    for (const f of ls(d)) {
      ...
      if (!j) continue;
      versions.push({
        version: `${sub}/${f.replace(/\.json$/, "")}`,
        label: `${label} — ${f.replace(/\.json$/, "")}`,
        path: path.join(d, f),
        generated: st ? st.mtime.toISOString() : null,
        agg_nll: j.agg_nll ?? null,
        quality: j.quality_score ?? j.quality ?? null,
  ...
  return {
    ...
    generator: "scripts/train/qlora/train_qlora_p5.py + scripts/train/eval/*",
    versions
  };
}
